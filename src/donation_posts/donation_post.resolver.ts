...
import { Args, Resolver, Query, Mutation, ResolveField, Parent, Subscription } from "@nestjs/graphql";
...
export class DonationPostResolver {
    ...
    async donationPosts(@Args('input') input: FetchDonationPostsInput): Promise<DonationPost[]> {
        return await this.donationPostService.fetchDonationPosts(input);
    }

    @Query(returns => DonationPost)
    async donationPost(@Args('input') input: FetchDonationPostInput): Promise<DonationPost> {
    ...
    async createDonationPost(@Args('input') input: CreateDonationPostInput): Promise<DonationPost> {
        const donationPost: DonationPost = await this.donationPostService.createDonationPost(input);
        this.pubSub.publish('donationPostAdded', { "donationPostAdded": donationPost });
        return donationPost;
    }
     ...
     */
    @Mutation(returns => DonationPost)
    async updateDonationPost(@Args('input') input: UpdateDonationPostInput): Promise<DonationPost> {
     ...
     */
    @Mutation(returns => DonationPost)
    async deleteDonationPost(@Args('input') input: DeleteDonationPostInput): Promise<DonationPost> {
        ...
        returns => DonationPost,
        {
            ...
            filter: async (payload, variables) => {
                ...
                if (input.type || input.bloodType) {
                    ...
                    if (input.bloodType && (input.bloodType !== addedDonationPost.bloodType)) {
                        return false;
                    }
                }
                return true;
    ...
    async donationPostAdded(@Args('input') input: DonationPostAddedInput) {
        return this.pubSub.asyncIterator('donationPostAdded');
    }
}